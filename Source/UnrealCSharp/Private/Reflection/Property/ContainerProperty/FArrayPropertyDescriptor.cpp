...
MonoObject* FArrayPropertyDescriptor::NewRef(void* InAddress) const
{
	...
	if (Object == nullptr)
	{
		...
		const auto ArrayHelper = new FArrayHelper(Property->Inner, InAddress, false, false);
...
MonoObject* FArrayPropertyDescriptor::NewWeakRef(void* InAddress, const bool bIsCopy) const
{
	if(bIsCopy)
	{
		...
		const auto ArrayHelper = new FArrayHelper(Property->Inner, CopyValue(InAddress), true, false);
