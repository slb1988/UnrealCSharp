...
MonoObject* FMapPropertyDescriptor::NewRef(void* InAddress) const
{
	...
	if (Object == nullptr)
	{
		...
		const auto MapHelper = new FMapHelper(Property->KeyProp, Property->ValueProp, InAddress, false, false);
...
MonoObject* FMapPropertyDescriptor::NewWeakRef(void* InAddress, const bool bIsCopy) const
{
	if(bIsCopy)
	{
		...
		const auto MapHelper = new FMapHelper(Property->KeyProp, Property->ValueProp, CopyValue(InAddress), true, false);
