...
MonoObject* FOptionalPropertyDescriptor::NewWeakRef(void* InAddress, const bool bIsCopy) const
{
	if(bIsCopy)
	{
		...
		const auto OptionalHelper = new FOptionalHelper(Property, CopyValue(InAddress), true, false);
	...
	else
	{
		...
		const auto OptionalHelper = new FOptionalHelper(Property, InAddress, false, false);
